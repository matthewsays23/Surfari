...
import { Clock, Users, Target, Trophy, RefreshCcw, AlertTriangle, Search, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function Activity() {
  ...
  const [leaders, setLeaders] = useState([]); // weekly top (for hero + quota panel)
  ...
  // NEW: full progress listing state
  ...
  // hydrate names/thumbs for any ids we see
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Kpi icon={Target} label={`Quota Met (${quotaTarget}m)`} value={`${summary?.quotaPct ?? 0}%`} />
      </div>

      {/* Existing small Quota Panel (top 8) */}
      <QuotaPanel
      ...
      {/* Recent + Leaderboard (as before) */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Leaderboard leaders={leaders} names={names} thumbs={thumbs} />
        <Recent recent={recent} names={names} thumbs={thumbs} />
      </div>
    </div>
  );
}

/* --- Quota Directory (search + paging) --- */
      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-2 text-sm rounded-lg border border-orange-200 focus:outline-none focus:ring-1 focus:ring-orange-300 bg-white"
          />
        </div>
      </div>
              ...
              <li key={u.userId} className="py-3 flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 h-2 rounded-full bg-orange-100 overflow-hidden">
                    <div
                      className={`h-full ${hit ? "bg-emerald-500" : "bg-orange-400"}`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
...
/* --- Smaller components reused from your current Activity UI --- */
function QuotaPanel({ leaders, names, thumbs, quotaTarget, nextResetAt }) {
  const resetIn = (() => {
    const diff = Math.max(0, nextResetAt.getTime() - Date.now());
    const s = Math.floor(diff / 1000);
...
function Recent({ recent, names, thumbs }) {
  return (
    <section className="rounded-2xl border border-orange-100 bg-white/95 p-5">
      <div className="flex items-center gap-2 mb-3">
...
/* small UI helpers from your version */
function Kpi({ icon: Icon, label, value }) { /* ...same as before... */ }
function RankBadge({ n }) { /* ...same as before... */ }
function AvatarFallback({ name = "?" }) { /* ...same as before... */ }
function Empty({ msg }) { /* ...same as before... */ }
function timeAgo(dateLike) { /* ...same as before... */ }
